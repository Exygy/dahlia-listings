...
module ApplicationHelper
  ...
  def dev_asset_paths
    ...
    Rails.application.assets.each_file do |f|
      next if f !~ %r{images/|json/} || %r{favicon/}.match?(f)
  ...
  def prod_asset_paths
    ...
    Rails.application.assets_manifest.assets.each do |f, hashpath|
      next if f !~ /jpg|png|svg|json/ || /favicon|apple\-icon|android\-icon/.match?(f)
      f = f.gsub('translations/', '') if %r{translations/}.match?(f)
